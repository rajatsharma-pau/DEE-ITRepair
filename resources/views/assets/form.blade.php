...
<div class="row">
    <div class="col-md-4 form-group"><label>College / Directorate</label><select id="asset_college_id" name="college_id" class="form-control"><option value="">Select</option>@foreach($colleges as $c)<option value="{{ $c->id }}" {{ old('college_id',$asset->college_id)==$c->id?'selected':'' }}>{{ $c->name }}</option>@endforeach</select></div>
    <div class="col-md-4 form-group"><label>Department / Office / KVK</label><select id="asset_department_id" name="department_id" class="form-control"><option value="">Select</option>@foreach($departments as $d)<option value="{{ $d->id }}" data-college="{{ $d->college_id }}" {{ old('department_id',$asset->department_id)==$d->id?'selected':'' }}>{{ $d->name }}{{ $d->place ? ' - '.$d->place : '' }}</option>@endforeach</select></div>
    <div class="col-md-4 form-group"><label>Note</label><input class="form-control" readonly value="If assigned to employee, department will be taken from employee current posting."></div>
</div>
<div class="row">
    ...
    <div class="col-md-3 form-group"><label class="required">Category</label><select name="asset_category" class="form-control" required>@foreach($categories as $c)<option value="{{ $c }}" {{ old('asset_category',$asset->asset_category)==$c?'selected':'' }}>{{ $c }}</option>@endforeach</select></div>
    <div class="col-md-3 form-group"><label class="required">Item Name</label><input name="item_name" class="form-control" required value="{{ old('item_name',$asset->item_name) }}"></div>
</div>
<div class="row">
...
<div class="row">
    ...
    <div class="col-md-3 form-group"><label class="required">Condition</label><select name="condition_status" class="form-control" required>@foreach($conditions as $c)<option value="{{ $c }}" {{ old('condition_status',$asset->condition_status)==$c?'selected':'' }}>{{ $c }}</option>@endforeach</select></div>
    <div class="col-md-3 form-group"><label class="required">Asset State</label><select name="asset_state" id="asset_state" class="form-control" required>@foreach($states as $s)<option value="{{ $s }}" {{ old('asset_state',$asset->asset_state)==$s?'selected':'' }}>{{ $s }}</option>@endforeach</select></div>
</div>
<div class="row">
