...
@section('content')
@php
    $canManageStore = isset($canManageStore) ? $canManageStore : \App\Support\StoreAccessScope::canManageStore(Auth::user());
@endphp
...
<div class="card mb-3">
    <div class="card-body">
        <form method="GET" class="form-row">
            ...
            @if(\App\Support\StoreAccessScope::canViewStore(Auth::user()))
                ...
                <div class="col-md-2 mb-2"><select name="department_id" class="form-control"><option value="">All Departments</option>@foreach($departments ?? [] as $d)<option value="{{ $d->id }}" {{ request('department_id')==$d->id?'selected':'' }}>{{ $d->name }}{{ $d->place ? ' - '.$d->place : '' }}</option>@endforeach</select></div>
...
<td>
    <a href="{{ route('assets.show',$a) }}" class="btn btn-sm btn-primary">View</a>
    @if(\App\Support\StoreAccessScope::canManageRow($a))
