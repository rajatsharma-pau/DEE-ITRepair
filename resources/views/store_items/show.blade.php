...
@section('content')
@php
    $canManageItem = \App\Support\StoreAccessScope::canManageRow($item);
@endphp
...
<tr><th>Reorder Level</th><td>{{ $item->reorder_level }}<br><small class="text-muted">Low stock alert level.</small></td><th>Location</th><td>{{ $item->location }}</td></tr>
<tr><th>College</th><td>{{ optional($item->college)->name }}</td><th>Department</th><td>{{ optional($item->department)->name }}</td></tr>
<tr><th>Description</th><td colspan="3">{{ $item->description }}</td></tr>
</table>
@if($canManageItem)<a href="{{ route('store-items.edit',$item) }}" class="btn btn-warning">Edit</a>@endif
</div></div></div>
<div class="col-md-4">
@if($canManageItem)
...
@else
<div class="alert alert-info">View only. Only the Storekeeper of this department or Superuser can update stock.</div>
@endif
</div>
</div>
